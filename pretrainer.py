...
class PreTrainer_bas_relief_AE(object):
	def __init__(self, config):
		...
		# build model
		print("Init Network...")
		self.network = im_bas_relief_network(
			self.ef_dim, self.gf_dim, self.z_dim, self.point_dim, self.camera_dim)
		self.network.to(self.device)

		# init validator
		self.validator = Validator(
			self.validate_dataloader, self.device, self.network)
		# self.network.double()
		...
		self.checkpoint_name = 'bas_relief_AE.model'
	...
	def train(self, config):
		...
		for epoch in range(0, config.epoch):
			...
			for n_iter, data in enumerate(self.train_dataloader):
				...
				_, net_out = self.network(rpts, None, pts, camera, is_training=True)
